...
# Criar dicionário auxiliar para nome dos meses
MESES = {
...
# Gerar lista de meses únicos ordenada por cod mes
meses_ordenados = (
    df[['cod_mes', 'mes']]
    .drop_duplicates()
    .sort_values('cod_mes')
...
# Criar selectbox com ordem correta
mes = st.selectbox("Selecione o mês", meses_ordenados)

filtro = df[(df['uf'] == estado) & (df['mes'] == mes) & (df['chegadas'] >= 10)]

fig = px.bar(filtro, x='pais', y='chegadas', color='via', title='Chegadas por País e Via')
st.plotly_chart(fig)
